...
def load_local_data_store(data_dir: str) -> Store:
    # build the store
    store = Store(base_dir=data_dir, protocol="file")
    return store
...
def _query_txG_from_parquets(
    ...
    def _map(df: pd.DataFrame, old_name: str, new_name: str):
        ...
        df2 = (
            df.merge(df_txid2row, left_on=old_name, right_on="txid", how="inner")  # map
            .rename(columns={"index": new_name})
...
def fit_scaler(graph_data: Dict, cached_features_dir: str, datastore) -> StandardScaler:
    ...
    for p in tqdm(graph_data):
        df_f = pd.read_parquet(
            datastore.open_file(
                os.path.join(cached_features_dir, f"features_{p}.parquet")
            )
        )
...
def augment_labels(
    ...
    semi_supervised_resample_negs: str = None,
    semi_supervised_resample_factor: int = None,
) -> pd.DataFrame:
    if semi_supervised == False:
        ...
        # dont do anything
        pass
    elif (semi_supervised_resample_negs == "random") or (
        ...
        for i in rng.choice(
            range(len(_idx)),
            size=min(len(_idx), _n * semi_supervised_resample_factor),
            replace=False,
        ):
...
def read_from_cache(
    features_partition_filepath: str, edges_partition_filepath: str, datastore: Store
) -> Tuple[pd.DataFrame, pd.DataFrame]:
    df_f = pd.read_parquet(datastore.open_file(features_partition_filepath))
    df_e = pd.read_parquet(datastore.open_file(edges_partition_filepath))
    return df_f, df_e


def generate_from_queries(
    df_l: pd.DataFrame, _partition: pd.DataFrame, feature_datastore: Store
) -> Tuple[pd.DataFrame, pd.DataFrame]:
...
def _load_data(
    ...
    assert (
        datastore.exists(labels_dir) or is_empty("labels", datastore) == False
    ), "labels missing"
    ...
    if refresh_cache:
        ...
        from lib.naming_corrections import TABLES_COLUMNS_DEFAULT, TABLES_V5_2_V4_RENAME
    ...
    df_p = (
        pd.read_parquet(datastore.open_file(partitions_dir))
    ...
    for sp, A in df_p.groupby("split"):
        graph_data[sp] = {x: None for x in sorted(A["index"])}
        labelled[sp] = {x: None for x in sorted(A["index"])}
        counters[sp] = Counter()
    ...
    for sp in splits:
        for p in tqdm(graph_data[sp]):
            ...
            features_partition_filepath = os.path.join(
                cached_features_dir, f"features_{p}.parquet"
            )
            edges_partition_filepath = os.path.join(
                cached_edges_dir, f"edges_{p}.parquet"
            )
            ...
            if (
                ...
                df_l = pd.read_parquet(datastore.open_file(labels_partition_filepath))
...
3
    ...
    if (DATA_LABEL_TRAIN in splits) and (scaler is None):
        scaler = fit_scaler(
            graph_data[DATA_LABEL_TRAIN],
    ...
    for sp in splits:
        for p in tqdm(graph_data[sp]):
            ...
            assert datastore.exists(
                features_partition_filepath
            ), f"Missing Features Dataframe, '{features_partition_filepath}'"
            assert datastore.exists(
                edges_partition_filepath
            ), f"Missing Edges Dataframe, '{edges_partition_filepath}'"
            df_f, df_e = read_from_cache(
                features_partition_filepath, edges_partition_filepath, datastore
            )

            X = df_f[feature_column_names].fillna(value=0.0).values
            ...
            # be all of them
            (labelled[sp][p],) = np.where(y != 2)
            # print(np.unique(labelled[sp][p]))

            counters[sp].update(y)
            graph_data[sp][p] = build_data(X=X, y=y, edges=df_e[["from", "to"]].values)

            # FIXME
            # return df_f, df_e, df_l, df_p
